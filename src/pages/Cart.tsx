...
function ImageModal({ image, title, onClose }: { image: string; title: string; onClose: () => void }) {
  ...
  return (
    <div
      ...
      onClick={onClose}
    >
      <div className="relative max-w-5xl max-h-[95vh] w-full flex flex-col items-center" onClick={(e) => e.stopPropagation()}>
        <button
          onClick={onClose}
...
export default function Cart() {
  const { cart, removeFromCart, updateQty, updateStock, updateCartItem } = useApp()
  const total = useMemo(() => cart.reduce((sum: number, i: CartItem) => sum + i.price * i.quantity, 0), [cart])
      ...
      try {
        for (const item of cart) {
          ...
          if (!item.description && res.data.description) {
            updateCartItem(item.productId, { description: res.data.description })
          }
        }
      } catch (error) {
        ...
        <>
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex items-center gap-3 mb-3">
                        <span className={`text-xs font-medium px-2 py-1 rounded ${
                          ...
                          {item.stock > 0 ? `Stock Available: ${item.stock}` : "Out of Stock"}
                        </span>
                      </div>
                      ...
                      <div className="flex flex-col sm:flex-row sm:items-center gap-3">
                        <div className="flex items-center gap-3">
                          ...
                          />
                          {item.stock > 0 && (
                            <span className="text-xs text-gray-500">(Max: {item.stock})</span>
                          )}
                        </div>
                        <div className="text-sm font-semibold text-gray-700">
                          Subtotal: <span className="text-lg text-gray-900">₹{item.price * item.quantity}</span>
                        </div>
                      </div>
                    </div>
          ...
          <div className="bg-white rounded-lg border-2 border-sky-200 p-6">
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
              ...
              <Link
                to="/checkout"
                className="bg-gradient-to-r from-sky-600 to-blue-600 text-white px-8 py-3 rounded-lg hover:from-sky-700 hover:to-blue-700 transition-all transform hover:scale-105 active:scale-95 font-semibold text-lg shadow-lg text-center"
              >
                Proceed to Checkout
              </Link>
            </div>
