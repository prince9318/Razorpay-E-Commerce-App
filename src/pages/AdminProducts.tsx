...
export default function AdminProducts() {
  ...
  const [form, setForm] = useState<Product>({ title: "", price: 0, description: "", image: "", stock: 0 })
    ...
    <div className="max-w-6xl mx-auto space-y-6">
      ...
      <div>
        ...
        <p className="text-gray-600 mt-1">
          {editingId ? "Update the product below" : "Add new products or edit existing ones"}
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-white rounded-xl border-2 border-sky-200 p-6 shadow-sm">
          ...
          <div className="space-y-4">
            ...
            <div>
              ...
              />
              {form.image && (
                <img src={form.image} alt="Preview" className="mt-2 h-20 w-20 object-cover rounded-lg border-2 border-gray-200" />
              )}
            </div>
            <div>
        ...
        <div className="space-y-4">
          <h3 className="text-lg font-semibold text-gray-900">Existing Products ({items.length})</h3>
          {loading ? (
              ...
              {items.map((p) => (
                <div
                  ...
                  className={`bg-white rounded-xl border-2 p-4 flex gap-4 shadow-sm hover:shadow-md transition-all ${
                    editingId === p._id ? "border-sky-500 ring-2 ring-sky-200" : "border-sky-200"
                  }`}
                >
                  <img
                    src={p.image || "https://via.placeholder.com/80?text=No+Image"}
                    alt={p.title}
                    className="h-16 w-16 sm:h-20 sm:w-20 object-cover rounded-lg border-2 border-gray-200 flex-shrink-0"
                  />
                  <div className="flex-1 min-w-0">
                    <div className="font-semibold text-gray-900 truncate">{p.title}</div>
                    <div className="text-gray-600 text-sm mt-0.5">₹{p.price}</div>
                    <div className="text-gray-500 text-xs mt-1">Stock: {p.stock}</div>
                  </div>
