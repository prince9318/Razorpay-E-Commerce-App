...
type RazorpayOptions = {
  ...
  order_id: string
  name: string
  handler: (response: { razorpay_order_id: string; razorpay_payment_id: string; razorpay_signature: string }) => Promise<void> | void
  theme?: { color?: string }
}
...
export default function Checkout() {
  ...
  const pay = async () => {
    setProcessing(true)
    try {
      const products = cart.map((c: CartItem) => ({ productId: c.productId, quantity: c.quantity, price: c.price }))
      const orderRes = await api.post("/orders", { products, totalAmount: total })
      await loadRazorpay()
      const createRes = await api.post("/payments/create-order", { amount: total, orderId: orderRes.data._id })
      const options = {
        ...
        <div className="space-y-6">
          <div className="bg-white rounded-lg border-2 border-sky-200 p-6 shadow-sm">
            ...
            <button
              ...
              className="w-full bg-gradient-to-r from-sky-600 to-blue-600 text-white px-6 py-3 rounded-lg hover:from-sky-700 hover:to-blue-700 disabled:opacity-50 disabled:cursor-not-allowed transition-all transform hover:scale-[1.02] active:scale-[0.98] font-semibold text-lg shadow-lg"
              onClick={pay}
            >
