...
type Product = { _id: string; title: string; price: number; description: string; image: string; stock: number }
...
function ExpandableDescription({ description }: { description: string }) {
  ...
  return (
    <div className="text-gray-600 text-xs leading-snug">
      <p>{expanded ? description : `${description.substring(0, maxLength)}...`}</p>
      <button
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="text-center py-6 sm:py-8">
        <h1 className="text-4xl sm:text-5xl lg:text-6xl font-bold bg-gradient-to-r from-sky-600 via-blue-600 to-indigo-600 bg-clip-text text-transparent mb-4 animate-fade-in">
          Discover Amazing Products
        </h1>
        <p className="text-gray-600 text-lg sm:text-xl">Shop the latest trends and best deals</p>
      </div>
      ...
      <div className="flex flex-col sm:flex-row gap-3 max-w-3xl mx-auto">
        <div className="flex-1 relative group">
          <div className="absolute inset-y-0 left-0 pl-4 flex items-center pointer-events-none">
            <svg className="h-5 w-5 text-gray-400 group-focus-within:text-sky-600 transition-colors" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      {loading && items.length === 0 ? (
        ...
        <div className="text-center py-20 bg-white rounded-2xl border-2 border-sky-200">
          ...
          <p className="mt-4 text-gray-600 text-xl font-medium">No products found</p>
            ...
            >
              ...
              <div className="relative overflow-hidden bg-gradient-to-br from-gray-50 to-gray-100 aspect-[4/3]">
                ...
                {p.stock === 0 && (
                  <div className="absolute inset-0 bg-black/60 flex items-center justify-center backdrop-blur-sm">
                    <span className="bg-red-500 text-white px-4 py-2 rounded-lg text-sm font-bold shadow-lg">Out of Stock</span>
                  </div>
                )}
              </div>
              ...
              <div className="p-3 flex flex-col flex-1">
                ...
                <div className="flex gap-2">
                  <button
                    onClick={() => addToCart({ productId: p._id, title: p.title, price: p.price, image: p.image, stock: p.stock, description: p.description })}
                    disabled={p.stock === 0}
